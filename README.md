# WATS Converter - Agilent ICT 3070 txt format

A WATS Client converter plugin for importing test data from Agilent ICT 3070 test system to WATS.
...
This is an example converter for Agilent ICT 3070 that only supports output in the same format as the example files. Different configurations of the test software will require an accordingly different converter.
...
| stationName       | test-machine          | If log is missing station name, use this one.                  |
| sequenceFile      | MainSequence          | If log is missing sequence name, use this one.                 |
...
* In TestICT3070Converter, fill in the path to the file you want to test the converter with. There are example files in the Data folder.
* Run SetupClient once, then you can run TestICT3070Converter as many times as you want.
